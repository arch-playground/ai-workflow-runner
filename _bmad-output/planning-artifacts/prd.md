...
- **FR67:** System restricts the agent's `bash` tool to a curated allowlist of read-only commands (grep/ls/find/cat/head/tail/wc/tree/rg) plus read-only git subcommands (log/show/diff/blame/shortlog/rev-list/status/tag/branch/…) so the agent can read source AND git history, while denying all mutating git ops (commit/push/pull/fetch/clone/merge/reset/checkout/remote) and credential-exposing ones (`git config`, reads of `.git/config`/`.git/credentials` — preserving the AGENT-03 checkout-token fix); allows `websearch`; default-denies `webfetch`; **denies file-write (`edit`/`write`) by default with a `writable_paths` opt-in allowlist**; and confines the agent (reads and bash) to the working directory (`external_directory: deny`) so it cannot read paths outside `GITHUB_WORKSPACE`. The Action's security permission rules are applied last (last-match-wins) so consumer config cannot weaken them.
- **FR68:** User can extend the bash command allowlist via a `bash_allow_patterns` input, scope the agent to a subdirectory via `agent_working_directory`, and restrict `webfetch` to a `webfetch_allowed_domains` allowlist (default empty ⇒ webfetch denied), enforced per-URL via OpenCode's permission engine (`OPENCODE_PERMISSION` env)
