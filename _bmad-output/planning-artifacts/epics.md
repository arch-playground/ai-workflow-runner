...
**AC:** `buildPermissionConfig` sets: **bash** = command-pattern allowlist (object form) permitting curated read-only extraction commands (`grep*`,`ls*`,`find*`,`cat*`,`head*`,`tail*`,`wc*`,`tree*`,`file*`,`rg*`,`git log*`,`git show*`,`git diff*`,`git blame*`,`git status*`) with a final `"*":"deny"`; a `bash_allow_patterns` input lets consumers extend the allowlist; **websearch** = `allow`; **webfetch** = `deny` by default (per-domain allowlist delivered in Story 13.9); **`external_directory` = `deny`** which confines BOTH reads and bash path-args (opencode's shell tool scans command path-args into `scan.dirs` and raises `external_directory` for out-of-root dirs — `tool/shell.ts:191/267`). **Agent confinement root** is set via `createOpencodeClient({ directory })` (→ `x-opencode-directory` header → `ctx.directory`), defaulting to `GITHUB_WORKSPACE`, with an optional `agent_working_directory` input to scope to a subdirectory (e.g. `/github/workspace/legacy-src`). So `cat /etc/passwd`, `grep -r /root`, `ls ~/.aws`, `cat auth.json` (outside root) are all refused; in-tree `grep`/`ls`/`find`/`cat` succeed. **Merge direction inverted** so Action security rules win under OpenCode's last-match-wins semantics (consumer `permission` applied first, Action rules overlaid last); the `handlePermissionAsked` handler must resolve a denied-bash/external_directory request to reject, never silently `'always'`. **Guard the worktree=/ gotcha:** a non-git workspace must still reject out-of-root paths (opencode special-cases `worktree==='/'`; add a test so we don't regress it). Verified: `curl|sh`/`rm`/installs denied; in-tree read commands work; out-of-root paths (incl. subdir confinement) refused; websearch works; consumer `opencode_config` cannot re-enable a denied command. (FR67, FR68) [Fixes AGENT-09 RCE, AGENT-02/03 on-disk reads; primary FINDING-5]
