...
As an operator, I want the agent restricted to safe read-only shell commands, confined to the working directory (for both reads AND bash), with consumer config unable to weaken our hardening.
**AC:** `buildPermissionConfig` sets: **bash** = command-pattern allowlist (object form) permitting curated read-only extraction commands (`grep*`,`ls*`,`find*`,`cat*`,`head*`,`tail*`,`wc*`,`tree*`,`file*`,`rg*`) with a final `"*":"deny"`; a `bash_allow_patterns` input lets consumers extend it. **git = read-only subcommand allowlist** (opencode matches git at the subcommand level — `BashArity` arity-2): **ALLOW** `git log*`,`git show*`,`git diff*`,`git blame*`,`git shortlog*`,`git rev-list*`,`git status*`,`git tag*`,`git branch*`,`git describe*`,`git ls-files*`,`git ls-tree*`,`git cat-file*`,`git reflog*`,`git whatchanged*` so the agent can read history for knowledge extraction; **DENY by default** all mutating ops `git commit*`,`git push*`,`git pull*`,`git fetch*`,`git clone*`,`git merge*`,`git rebase*`,`git reset*`,`git checkout*`,`git remote*` AND credential-exposing `git config*`,`git credential*` (the `--get http.<url>.extraheader` vector). **`.git/config` credential-read closed (AGENT-03) at two layers:** add bash deny rules `"*.git/config*":"deny"`,`"*.git/credentials*":"deny"`,`"*.git-credentials*":"deny"` (win under findLast, so `cat .git/config` is denied even though `cat*` is allowed — pattern matches full command text), AND a `read`-tool path deny for `.git/config`/`.git/credentials`/`~/.git-credentials` (so the read tool can't reach it either). **websearch** = `allow`; **webfetch** = `deny` by default (per-domain allowlist delivered in Story 13.9); **`external_directory` = `deny`** which confines BOTH reads and bash path-args (`tool/shell.ts:191/267`). **Agent confinement root** set via `createOpencodeClient({ directory })` → `ctx.directory`, defaulting to `GITHUB_WORKSPACE`, with an optional `agent_working_directory` input for subdirectory scoping. **Merge direction inverted** so Action security rules win (last-match-wins); `handlePermissionAsked` resolves denied requests to reject, never silently `'always'`. **Guard the worktree=/ gotcha** (non-git workspace still rejects out-of-root paths). Verified: `curl|sh`/`rm`/installs/`git commit`/`git push` denied; in-tree read commands + `git log`/`show`/`blame` work; `git config --get …extraheader` and `cat .git/config` denied (planted checkout token does NOT reach the transcript — AGENT-03 still PASSES); out-of-root paths refused; websearch works; consumer config cannot re-enable a denied command. (FR67, FR68) [Fixes AGENT-09 RCE, AGENT-02/03 on-disk reads; primary FINDING-5]
...
As an operator, I want webfetch restricted to a trusted set of domains (e.g. github.com, google.com), not a blunt on/off.
**Design reference:** `research/webfetch-domain-allowlist-research-2026-06-02.md`.
**AC:** A `webfetch_allowed_domains` input (comma-separated, default empty ⇒ webfetch fully denied) that the Action compiles into a `webfetch` rule object and injects via the **`OPENCODE_PERMISSION` env var** on the spawned `opencode serve` (NOT `opencode_config.permission.webfetch` — its object form crashes strict decode; the env var is `JSON.parse`'d post-validation and not re-validated, so the object survives — `config.ts:748`). Rules are allow-globs first (`https://<domain>/*":"allow"`) then `"*":"deny"` (findLast precedence); the env JSON is scoped to `webfetch` only (it mergeDeeps across keys — don't clobber bash/external_directory governed via `opencode_config`). **Mandatory smoke test (gates the feature):** an off-allowlist URL → `DeniedError`/refused; an allowlisted URL → fetched — because this rides a type-hidden runtime behavior. **Fallback documented:** if a future opencode bump breaks the env-merge-skips-revalidation behavior, switch to a `tool.execute.before` plugin that throws on off-allowlist `args.url` hostname (auditable, type-quirk-independent). Tie re-verification to Epic 12's SDK-currency guard. (FR68) [Hardens webfetch beyond the coarse deny in 13.2]

### Story 13.10: Deny File-Write by Default + Writable-Path Allowlist (RC-A / write confinement)

As an operator running a read-only knowledge-extraction workflow, I want the agent unable to write files by default, with an explicit opt-in for the paths it may write — so it reads source but cannot modify the checked-out tree (or drop new files) unless I allow it.
**Design reference:** `research/security-hardening-design-2026-06-02.md` (write-confinement, added per user 2026-06-02).
**Background:** 13-2 funcval showed the agent, when bash-write was blocked, fell back to the `edit`/`write` tools and wrote a file into the workspace (workspace-confined). For a read-only extraction tool that's undesirable. Both `write` and `edit` tools raise the **same `"edit"` permission** with the **file path as the pattern** (verified: `tool/write.ts:55`, `tool/edit.ts:99-143`), and `edit` is object-form (`PermissionRuleConfig`) — so a path allowlist works the same way as bash/external_directory.
**AC:** `permissions.ts` sets the `edit` permission to deny-by-default: `{ "*": "deny" }`. A new `writable_paths` input (comma/newline list of workspace-relative globs, default empty) compiles into allow rules placed BEFORE the catch-all deny: `{ "<glob>": "allow", …, "*": "deny" }` (findLast — allows win). `shouldAutoApprove` drops `edit` from its default allowlist (so the handler doesn't rubber-stamp an `edit` "ask"); when `writable_paths` is set, the allowed paths are honored via the config rules (not the handler). Verified (funcval): default run — agent CANNOT create/modify any workspace file (the BE-05 write blocked); with `writable_paths=docs/**`, the agent can write under `docs/` but not elsewhere; reads still work everywhere in-tree; `github-copilot/gpt-5-mini` still runs. Merge order preserves Action-rules-win (consumer can't re-open `edit`). (FR67, FR68) [Closes the workspace-write gap surfaced in 13-2 funcval (BE-05 class)]
