...
- Never share `auth.json` between jobs via artifacts — write it fresh per job
...
**Notes:**
- Add a cleanup step with `if: always()` if the config contains sensitive data: `run: rm -f config.json`
...
| Copilot `auth.json` | Write + cleanup steps in **each job** that uses `auth_config:` |
